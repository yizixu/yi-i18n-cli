...
import { translateSingle } from './translate-single'
...
program
  ...
  .option('-t, --translate <file> <from> <to> [single]', '翻译国际化文件（国际化文件地址，源语言，目标语言，是否逐个翻译）')
  .option('-s, --sync <file> <targetFile>', '同步国际化文件（源文件地址， 目标文件地址）')
  .parse(process.argv)
...
if (options.translate) {
  spinning('翻译文件', async () => {
    if (options.translate === true) {
      console.log('请按格式输入：--translate <file> <from> <to> [single]')
      return false
    }
    if (args[2] === 'single') {
      return await translateSingle(options.translate, args[0], args[1])
    }
...
if (options.sync) {
  spinning('同步文件', () => {
    if (options.sync === true) {
      console.log('请按格式输入：--replace <file> <targetFile>')
      return false
    }
    return sync(options.sync, args[0])
  })
}
